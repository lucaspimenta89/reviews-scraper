defmodule ReviewsScraper.ReviewsParserTest do
  ...
  describe "ReviewsParser - tests for single review" do
    ...
    test "get_review_rating/2", %{ review_html_tree: review_html_tree, review: %{ rating: rating } } do
      assert %Review{ rating: ^rating } = ReviewsParser.get_review_rating(%Review{}, review_html_tree)
    end
    ...
    test "get_review_author/2", %{ review_html_tree: review_html_tree, review: %{ author: author} } do
      assert %Review{ author: ^author } = ReviewsParser.get_review_author(%Review{}, review_html_tree)
    end

    test "get_review_content/2", %{ review_html_tree: review_html_tree, review: %{ content: content } } do
      assert %Review{ content: ^content } = ReviewsParser.get_review_content(%Review{}, review_html_tree)
    end

    test "get_individual_rates/2", %{ review_html_tree: review_html_tree, review: %{ individual_rates: individual_rates} } do
      assert %Review{ individual_rates: ^individual_rates } = ReviewsParser.get_individual_rates(%Review{}, review_html_tree)
    end

    test "get_employees_reviews/2", %{ review_html_tree: review_html_tree, review: %{ employees_reviews: expected_employees_reviews } } do
      assert %Review{ employees_reviews: ^expected_employees_reviews } = ReviewsParser.get_employees_reviews(%Review{}, review_html_tree)
    end
