defmodule ReviewsScraper.CLITest do
  ...
  describe "CLI - tests" do
    ...
    test "main/1", %{reviews_html: reviews_html, review: review} do
      with_mock(HTTPoison,
        get: fn _url -> {:ok, %HTTPoison.Response{status_code: 200, body: reviews_html}} end
      ) do
        assert output =
                 capture_io(fn ->
                   CLI.main(%{})
        ...
        expected_review = %{
          ...
          employees_reviews:
            Enum.map(review.employees_reviews, fn %{name: name, rating: rating} ->
              %{name: name, rating: rating}
            end),
          individual_rates:
            Enum.map(review.individual_rates, fn %{name: name, rating: rating} ->
              %{name: name, rating: rating}
            end),
